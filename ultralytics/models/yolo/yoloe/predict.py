...
class YOLOEVPDetectPredictor(DetectionPredictor):
    ...
    Attributes:
        ...
        prompts (dict): Raw visual prompts containing class indices and bounding boxes or masks.
        prompt_tensor (torch.Tensor | None): Encoded visual prompts for the current inference batch.
    ...
    def set_prompts(self, prompts):
        ...
        Args:
            prompts (dict): Dictionary containing class indices and bounding boxes or masks. Must include a 'cls' key
                with class indices.
        """
        self.prompts = prompts.copy() if isinstance(prompts, dict) else prompts
        self.prompt_tensor = None
        self.base_tpe = None
        self.base_pe = None

    def set_base_text_prompts(self, base_tpe: torch.Tensor | None, base_pe: torch.Tensor | None = None):
        """Cache base-class prompts for hybrid text+visual prompt inference."""
    ...
    def pre_transform(self, im):
        ...
        if not isinstance(self.prompts, dict):
            raise TypeError(
                f"Expected raw visual prompts to be a dict before preprocessing, but got {type(self.prompts).__name__}."
            )

        bboxes = self.prompts.get("bboxes")
        masks = self.prompts.get("masks")
        ...
        self.prompt_tensor = prompts.half() if self.model.fp16 else prompts.float()
        return img
    ...
    def inference(self, im, *args, **kwargs):
        ...
        """
        return super().inference(im, tpe=self.base_tpe, base_pe=self.base_pe, vpe=self.prompt_tensor, *args, **kwargs)
    ...
    def get_vpe(self, source):
        ...
        for _, im0s, _ in self.dataset:
            ...
            return self.model(im, vpe=self.prompt_tensor, return_vpe=True)
