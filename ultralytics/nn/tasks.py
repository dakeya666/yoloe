...
class YOLOEModel(DetectionModel):
    ...
    def predict(
        ...
        Args:
            ...
            base_pe (torch.Tensor, optional): Precomputed base-class prototypes for hybrid inference.
        ...
        for m in self.model:  # except the head part
            ...
            if isinstance(m, YOLOEDetect):
                ...
                if return_vpe:
                    ...
                    assert not self.training
                    return vpe
                text_pe = base_pe if base_pe is not None else m.get_tpe(tpe)
                cls_pe = self.get_cls_pe(text_pe, vpe).to(device=x[0].device, dtype=x[0].dtype)
    ...
    def loss(self, batch, preds=None):
        ...
        if not hasattr(self, "criterion"):
            ...
            visual_prompt = visuals is not None and (not isinstance(visuals, torch.Tensor) or visuals.numel() > 0)
            self.criterion = (
        ...
        if preds is None:
            ...
            preds = self.forward(
                batch["img"],
                tpe=None if visual_prompt else batch.get("txt_feats", None),
                vpe=visuals if visual_prompt else None,
            )
        return self.criterion(preds, batch)
