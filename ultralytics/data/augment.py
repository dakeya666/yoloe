...
class LoadVisualPrompt:
    ...
    @staticmethod
    def _normalize_category(category: int | str | list | tuple | np.ndarray | torch.Tensor) -> torch.Tensor:
        """Convert category labels to a 1D integer tensor.

        String labels are remapped to contiguous ids in first-appearance order so visual prompts can be built from
        human-readable class names as well as numeric ids.
        """
        if isinstance(category, torch.Tensor):
            return category.to(dtype=torch.int).view(-1)

        if isinstance(category, np.ndarray) and np.issubdtype(category.dtype, np.number):
            return torch.from_numpy(np.atleast_1d(category).astype(np.int64, copy=False)).to(dtype=torch.int)

        values = category.tolist() if hasattr(category, "tolist") and not isinstance(category, (str, bytes)) else category
        if isinstance(values, (str, bytes)) or not isinstance(values, (list, tuple)):
            values = [values]

        normalized, value_to_idx = [], {}
        for value in values:
            ...
            if isinstance(value, (str, bytes)):
                ...
                normalized.append(value_to_idx[value])
            else:
                normalized.append(int(value))
        return torch.tensor(normalized, dtype=torch.int)
