...
def test_load_visual_prompt_string_categories():
    """Test visual prompt generation with string category labels."""
    from ultralytics.data.augment import LoadVisualPrompt

    visuals = LoadVisualPrompt().get_visuals(
        ["novel_defect_a", "novel_defect_b", "novel_defect_a"],
        (64, 64),
        bboxes=np.array([[0, 0, 16, 16], [16, 16, 32, 32], [32, 32, 48, 48]], dtype=np.float32),
    )

    assert visuals.shape[0] == 2
...
def test_yoloe(tmp_path):
    ...
    repeated_results = model.predict(
        [SOURCE, SOURCE],
        visual_prompts=visuals,
        predictor=YOLOEVPSegPredictor,
    )
    assert len(repeated_results) == 2
